...
export const updateStatusLevel = async (req: Request, res: Response) => {
    const { idUser, idLevel, status, score } = req.body;

    try {
        if (!idUser || !idLevel || !status || score === undefined) {
        ...
        console.error(error);
        return res.status(500).json({ message: "Hubo un error al actualizar el estado del nivel y el puntaje." });
    }
};
